...
- The `nora_gpio` core sources are vendored under `src/hal_gpio/`.
- The `nora_gpio_event` sources are vendored under `src/hal_gpio/`.
- The GPIO core and event layer remain synchronized from `nora-hal-dspic33ak-gpio`,
